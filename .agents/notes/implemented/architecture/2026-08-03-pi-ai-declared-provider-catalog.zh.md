...
默认 catalog 来源是路由键。部署专用路由通过不同端点或凭据服务某个已安装提供方的模型时，`catalogProvider` 可以显式点名该提供方。选定的 catalog 提供模型、端点、协议与 auth 默认值；物化模型和构建出的 `Provider` 仍以路由键作为请求身份。空值或不存在的来源会让解析失败。系统不会根据模型 id 猜测，因为网关可以用相同 id 暴露不同能力。

- `catalog.ts` 把选定的已安装 catalog 合并到 profile 自身条目之下。profile 的 `models` 列表*替换*该 catalog（列表缺席或为空则原样服务），每个条目从同 `id` 的已安装模型继承自身未设置的字段。只有 harness 会消费的字段可配置——`id`、`name`、`contextWindow`、`maxTokens`；[[2026-08-08-pi-ai-per-model-reasoning-declarations]] 之后加入了 `reasoningEfforts` 与 `compat`，当初「推理（reasoning）沿用已安装条目或直接缺席」的立场也在那里被重新审视（孤立的能力布尔量仍被拒绝；带 wire 拼写的逐档位完整声明没有它那个问题）。输入模态后来被开放，形态是条目上的 `input` 加路由级 `defaultInput`——图片准入点使得「未被报告的模态」变成部署无法解除的拒绝之后（[[2026-08-12-pi-ai-route-default-input-modalities]]）；当初「没有任何读取方」那句论证描述的其实是 `llm-deepseek` 的序列化器，而不是这条路由，它的转换器能携带图片。定价仍因原有理由不出现在配置面：`replay.ts` 把 pi-ai 的成本元数据清零，且没有任何消费方报告开销。物化时以已安装条目铺底、再覆盖已配置的字段，而不是逐字段枚举结果：枚举式重建会静默丢弃本包未建模的每一个 `Model` 字段——`headers` 就是这样从某条 nvidia 路由上消失过一次。
- `provider.ts` 构造路由的 `Provider`。保持选定 catalog 协议不变的路由会**复用**该已安装提供方，并替换其身份与 `getModels()`；其余路由都由 `createProvider()` 基于一张协议表构造，表中条目正是 pi-ai 自己的提供方工厂所用的 `@earendil-works/pi-ai/api/*.lazy` factory。该表刻意窄于 pi-ai 的完整 API 集合——只保留 profile 能用密钥、端点与标头完整描述的协议，因此 Bedrock（SigV4 加 region）、Vertex（project、location、ADC）、Azure（提供方环境加 api-version）与 Codex（OAuth）不在其中，而不是被当作无法认证的路由提供出去。catalog 路由仍可经选定的 provider 抵达它们；被拒的只有显式覆盖。
...
解析失败得响亮，并点名出问题的路由与模型：选定 catalog 未描述的模型会回落到该路由自己的 `defaultContextWindow`／`defaultMaxTokens`，因此只公布 id 的列表也能得到可服务的路由；没有 catalog 来源的路由需要 `api`、`baseURL` 和非空的 `models` 列表。由于构造出的 `Provider` 是解析结果的一部分，协议或模型出错时最后可用的路由集合会继续服务——与此前坏的 settings 快照的行为完全一致。
...
- **根据模型 id 或所选协议猜测 catalog 来源。** 配置更短，但两者都不能证明能力等价：catalog 会重叠，私有网关也可能复用官方 id 却只暴露较少模态或推理档位。因此来源必须是显式路由事实。
...
配置一个提供方不再取决于 pi-ai 的发布节奏。网关、自建服务，或比锁定 catalog 更新的模型，都是一次 `settings.yaml` 编辑，陈旧的上下文窗口也能就地更正。服务已安装提供方模型的自定义路由不再重述这些模型的能力；它只需点名一次来源，便会随锁定的 pi-ai catalog 继承更新。废弃的 `/compat` 导入已经消失，因此 pi-ai 删除它不再是破坏性事件。`defaultMaxTokens` 现在会在部署明确给出时从配置中传入，不会从 catalog 元数据里发明一个上限。
