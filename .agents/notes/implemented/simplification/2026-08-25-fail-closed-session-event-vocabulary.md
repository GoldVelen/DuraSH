...
One narrow exception ships with the guard: the `INERT_LEGACY_EVENT_TYPES` set lets the three workflow run-state mirrors (`runs/dispatched`, `workflow/start`, `workflow/change`) pass through reconstruction untouched. The 0.1.1-era workflow orchestrator appended them to the session stream before run durability moved to the storage `runs` unit; their declaring type pinned `modelVisible: false`, so they never carried a fact a reconstructed model request reads. They pass through rather than drop because `seq` must stay contiguous from 0 — the `seq = log.length` contract forbids removing stored records — and event folds ignore types outside their branches by the merge-extensible default. They stay outside the generated known set because this build never re-emits them. This is the case the rejected skip assertion lacked: a defined event class the writer proves informational, allowed at one choke point, with every other unknown type still refusing.
