...
源码 profile 解析使用真实文件系统查找路径，使 TypeScript hook 与 profile 加载的插件共享工具模块身份。源码启动兼容性测试通过实际 CLI 验证该身份。

浏览器进程检查选择与已验证客户端构建一致的 profile。浏览器测试允许复用空白对话，在自适应目录选择器之外精确检查启动批次，并记录已经实现的账号登录区。PowerShell 重放 header 描述各隔离场景实际启用的工具；已提交的 Session 各代历史保持不变。
