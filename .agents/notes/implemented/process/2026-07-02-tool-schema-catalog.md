...
Booting has a cost the AST pass did not: there is no source declaration set to enumerate, so a new tool package could simply be forgotten. A **completeness guard** restores the guarantee — `assertManifestComplete` globs matching packages that contain `package.json`, then hard-errors if any is absent from the generator's boot list. The default glob is `packages/*/tool-*`. DuraSH product tools use a separate boot list and glob. A new matching package fails the generator, and therefore `doc-sync`, until it is registered. This is the same structural property the cordis generator gets for free from enumerating source, re-created for a boot-based generator.
...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `dsh-tool-bash` (`bash`), `dsh-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `dsh-tool-subagent` (`subagent`). Example-only tools are excluded. DuraSH `durash-tool-*` packages, including `dsh-tool-reliability` (`dsh_reliability_handoff`), are harvested by `scripts/gen-durash-tool-catalog.ts` with inert service identities because schema harvest never executes the tool.
...
- The catalog cannot drift: a tool schema change the committed file doesn't reflect fails `verify-tool-catalog` in `doc-sync` and CI. A new `tool-*` package not added to the upstream boot list fails that completeness guard. DuraSH `durash-tool-*` packages use `scripts/gen-durash-tool-catalog.ts` and fail `verify-durash-tool-catalog` if they are missing from that list.
