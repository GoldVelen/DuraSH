...
启动有一项 AST 遍历不存在的代价：没有源码声明集合可供枚举，新工具包可能被遗忘。一个**完整性守卫**恢复了这项保证——`assertManifestComplete` 对匹配且含 `package.json` 的包进行 glob，若有任何一个不在生成器的启动清单中则直接报错。默认 glob 是 `packages/*/tool-*`。DuraSH 产品工具使用单独的启动清单和 glob。新的匹配包在注册之前会导致生成器失败，进而导致 `doc-sync` 失败。这与 Cordis 生成器通过枚举源码免费获得的结构性属性相同，只是为基于启动的生成器重新实现了一遍。
...
`packages/*/tool-*` 下已发布的产品工具包，每个都使用默认配置启动，包括 `dsh-tool-bash`（`bash`）、`dsh-tool-jobs`（`job_output`、`job_list`、`job_kill`）和 `dsh-tool-subagent`（`subagent`）。仅供示例使用的工具不在范围内。DuraSH `durash-tool-*` 包，包括 `dsh-tool-reliability`（`dsh_reliability_handoff`），由 `scripts/gen-durash-tool-catalog.ts` 采集，并使用惰性服务身份，因为 schema 采集绝不会执行工具。
...
- 目录不会发生漂移：提交文件未反映的工具 schema 变化会使 `doc-sync` 和 CI 中的 `verify-tool-catalog` 失败。新增的 `tool-*` 包若未加入上游启动清单，会使该完整性守卫失败。DuraSH `durash-tool-*` 包使用 `scripts/gen-durash-tool-catalog.ts`，缺失时会使 `verify-durash-tool-catalog` 失败。
