...
The inherited Issue lifecycle, pull-request policy, trusted live-API wheel checks, and official `@deepseek-ai/dsh` release jobs declare `deepseek-harness/deepseek-harness` as their only applicable repository. A downstream repository skips those jobs before checkout, token creation, policy execution, live-API preflight, or pack-and-install publication rehearsal; it does not report a pass for Project or npm-release state that it cannot own.
...
- Generalizing the upstream Issue Project policy, trusted live-API wheel checks, or the official `@deepseek-ai/dsh` npm release rehearsal across downstream repositories: rejected because the App credentials, Project fields, lifecycle actor, trusted API secret, release tags, and public package namespace all belong to the upstream repository.

## Consequences

The repository has a small, visible downstream ownership surface, a DuraSH-default source entry, a DuraSH-specific build-and-composition check, and a repeatable path to the latest verified upstream. The automated merge composes only deterministic pairing records; it does not turn semantic conflicts green. Organization-owned Issue checks, trusted live-API wheel checks, and the upstream official npm-release rehearsal are visibly not applicable in DuraSH instead of failing for missing credentials or validating a different repository. Upstream development and official artifact commands stay explicit instead of inheriting the product default. Public Actions and branch protection still require configuration after the GitHub repository is created. The brand/profile migration does not close the reliability-engine migration; that remains the next product milestone and cannot be advertised as complete.
