...
`dsh-agent-instructions` 向 agent（智能体）提供来自用户全局文件和项目级文件的工作区指引；这些文件均与 `AGENTS.md` 兼容。它为第一次请求加载适用的指令链。“全局规则”设置编辑同一个 Host 文件，并在请求准入前刷新已有会话。它不会持续监视外部编辑：成功的文件系统操作会发现新适用的嵌套文件，并让后续变更或移除可见；恢复会话也会对账基线。`dsh-base` 默认启用此行为，profile 可以禁用。字节预算限制注入的上下文：较宽泛的文件先被省略，最具体的文件最后被截断，空指令链不添加任何内容。
...
第一次请求包含一条持久基线消息：先是 Host 上的用户全局 `$DSH_HOME/AGENTS.md`，再按从宽泛到具体的顺序包含文件系统提供方的项目指令链——从项目根目录到会话工作目录的每个目录中所有现有候选文件。去除首尾空白后内容一致的同级文件只渲染一次，因此复制了 `AGENTS.md` 的 `CLAUDE.md` 不会被重复加载。当成功的 `read`、`write` 或 `edit` 调用到达更深的目录后，下一次请求会包含新适用的指令文件；已改变的文件会替换其内容，消失或成为较早候选文件重复项的文件会产生移除通知。
...
设置 → 全局规则读取本插件依次通过 `dshHome`、`$DSH_HOME`、`~/.dsh` 解析的 Host 文件原文。文件是唯一可编辑真源：打开页面不会创建文件，保存保持提交的 UTF-8 文本。写入准备前和替换前的版本检查拒绝过期编辑；原子替换前显式恢复权限位，不受进程 umask 影响。符号链接可以读取，但不能通过此编辑器保存。

保存成功仅确认文件已写入。已有会话下一次请求被接纳前，插件重建基线与此前已发现的嵌套作用域，为旧指令节点记录 `user/message` surface 替换，并开始新的请求系列。旧规则正文从派生历史中移除；相同正文的保存不会重复注入。运行中的工具与已发出的请求继续执行。新会话首次请求读取已保存文件。进程内工作流子代理使用同一插件；外部代理提供方仍使用各自的指令机制。禁用此插件或文件系统提供方会阻止加载，源文件和渲染预算也可能省略规则。某次请求加载了什么，应由持久指令消息与请求上下文确认，不能以保存结果代替。
...
| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | 插件入口：pre-step 和 request-context 监听器、`tools/result` touch 跟踪、inbox 组合 |
| [`src/global-rules.ts`](src/global-rules.ts) | Host 文件编辑器、版本检查与保存代次 |
...
文件系统工具更新追加在可复用前缀之后。设置保存会替换已有指令节点并开始新的请求系列，因此从第一个改变的指令节点起使可复用前缀失效。
...
- **外部写入方仍可能与最终替换竞争**——保存会检测编辑期间和文件准备期间的更改。不遵从写锁的进程仍可能在最终版本检查与重命名之间修改文件；文件系统不为此操作提供原子的比较并交换保证。
- **外部刷新没有 watcher**：设置保存会明确刷新下一次被接纳的请求；外部编辑会在下一次成功的第一方 `read`、`write` 或 `edit` 时、恢复对账可见基线时，或进入步骤的 pre-step 恢复被遮蔽基线时可见。
