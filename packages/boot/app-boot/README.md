...
A profile is how one dsh installation ships different app surfaces: `web`, `durash`, `headless`, `acp`, `sdk`, and `sdk-minimal` start distinct compositions from the same launcher. A profile lives at `$DSH_HOME/profiles/<name>` and combines installable bundles, its own `cordis.patch.yml`, and `patchReload: live | startup`; omitted reload policy keeps the historical `live` default for custom profiles. The shipped `web` and `durash` templates use live reload, while the other shipped templates apply patches only at startup. `durash` adds the product-owned overlay after the upstream Web stack, `sdk-minimal` names only its standalone bundle, and the remaining templates retain base-plus-mode stacks. `dsh plugin` creates custom profiles, and a missing bundle or one without a patch declaration fails startup loudly.
