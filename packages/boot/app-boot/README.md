...
A profile is how one dsh installation provides different app surfaces from the same launcher. The published dsh package auto-initializes `web`, `headless`, `acp`, `sdk`, and `sdk-minimal`; a DuraSH source checkout passes a separate source-owned `durash` template to the same loader. A profile lives at `$DSH_HOME/profiles/<name>` and combines installable bundles, its own `cordis.patch.yml`, and `patchReload: live | startup`; omitted reload policy keeps the historical `live` default for custom profiles. The published `web` template and source-only `durash` template use live reload, while the other published templates apply patches only at startup. `durash` adds the private product overlay after the upstream Web stack, `sdk-minimal` names only its standalone bundle, and the remaining templates retain base-plus-mode stacks. `dsh plugin` creates custom profiles, and a missing bundle or one without a patch declaration fails startup loudly.
