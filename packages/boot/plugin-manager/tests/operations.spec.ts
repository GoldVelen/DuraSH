...
it('initializes an installation-owned profile template without replacing an existing selection', async () => {
  const { home, context } = fixture()
  const profile = 'durash'
  const dir = join(home, 'profiles', profile)
  const bundles = ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app', '@durash/dsh-web-profile']
  command.run.mockImplementation(() => result(0, ''))
  await runPluginCommand({ ...context, profile, profileTemplates: { [profile]: { bundles } } }, ['root'], {
    execution: 'service', outputBytes: 100,
  })
  expect(readProfileManifest('test', dir).dsh?.profile?.bundles).toEqual(bundles)

  await runPluginCommand({ ...context, profile, profileTemplates: { [profile]: { bundles: ['replacement'] } } }, ['root'], {
    execution: 'service', outputBytes: 100,
  })
  expect(readProfileManifest('test', dir).dsh?.profile?.bundles).toEqual(bundles)
})
