...
import {
  ...
  resolveBundleDir, resolveProfileDir, loadOverlayPatches, type ProfileManifest, type ProfileTemplateMap,
...
export interface PackageOperationContext {
  ...
  cwd: string
  home?: string
  /** Installation-owned templates used when initializing an absent profile. */
...
export async function runPluginCommand(
  ...
  return withFileLock(join(dir, 'package.json'), async () => {
    if (!existsSync(join(dir, 'package.json'))) {
      const template = (context.profileTemplates ?? PROFILE_TEMPLATES)[context.profile]
