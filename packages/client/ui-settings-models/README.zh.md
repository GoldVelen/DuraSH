...
`dsh-client-ui-settings-models` 是 dsh 网页客户端的「模型」设置页：用户配置 API 密钥（以只写方式存入 profile 的凭据引用）、用订阅账号登录、编辑各提供方的模型列表，并手工声明自定义 pi-ai 路由；页面以提供方行加单一编辑卡片呈现。该页面把提供方目录、设置文档与凭据描述合并为一个共享快照，因此行的状态在三个方面始终一致。它还会带首次运行的用户走两个有序弹窗——版本化内测声明，以及按条件显示的官方 DeepSeek 凭据步骤。
...
「新增」流程是一张承载休眠目录提供方选择框的卡片——裸挂载的 `llm-pi-ai` 在任何路由存在之前就能提供其完整的已安装 catalog。**添加自定义提供方**声明一条 pi-ai 不提供的路由；创建卡片会索要唯一的 **Provider ID**、端点、协议与至少一个可唯一识别的模型，因为没有东西能为它们兜底。端点必须是可解析的 HTTP 或 HTTPS URL；localhost、IPv4 与 IPv6 字面地址以及自定义端口仍然有效。语法错误会在字段处阻止询问与创建，请求失败则继续作为独立的提供方错误显示。**获取可用模型**通过 `llm/discoverModels` 显式请求刷新表单所示端点的模型列表；打开编辑器仍只读取已安装目录，不访问端点。刷新失败会显示 Host 诊断并保留草稿模型列表。回复打开可搜索选择器；**添加所选**将候选复制到草稿，点击应用或创建才保存。每个选中候选会在提供方公布相应信息时，把 id、显示名、上下文窗口、最大输出 token 数、输入类型和推理档位映射复制进可编辑行；已有行即使再次选中，也会保留用户调整过的全部值。搜索会匹配模型 id 与可选显示名称，且不会清除隐藏项的勾选状态。**全选**会加入可见结果，而**取消全选**会清空全部勾选，以免意外采用隐藏结果。只有用户层单独携带某行时，该行才可删除（删除会恢复组合基线），其确认对话框会指名该提供方。

### 账号登录

**账号登录**区域列出宿主中带 OAuth 方法的每个登录流——ChatGPT Plus/Pro、Grok/X、Claude Pro/Max，以及 pi-ai 适配器注册的其他订阅登录。仅 API 密钥的目录项留在添加表单里。**登录**在宿主侧启动尝试，区域渲染其进度：要打开的页面、要输入的设备码，以及唯一的待答提问，可就地作答或跳过；**取消登录**撤回尝试。尝试状态存于宿主，重新加载页面会重新加入正在进行的尝试；已结束的尝试保持可见，直到下一次开始。登录成功后，本页写入与添加卡片相同的空白原生认证 profile（路由随之注册，模型进入对话选择器），并打开该提供方的模型目录编辑器。没有 OAuth 登录流时此区域不渲染，包括未挂载 authorization 服务的组合。Client 通过 `ctx.inject` 绑定该命名空间，因此从未挂载它的 fixture 仍能加载本页；未注入的 `ctx.remote.authorization` 访问器在命名空间已经存在时也会抛错。
