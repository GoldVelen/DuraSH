...
**账号登录**区域列出宿主注册的每个登录流——pi-ai 适配器内置登录的订阅提供方（ChatGPT Plus/Pro、Grok/X、Claude Pro/Max 及其他 OAuth 目录项）。**登录**在宿主侧启动尝试，区域渲染其进度：要打开的页面、要输入的设备码，以及唯一的待答提问，可就地作答或跳过；**取消登录**撤回尝试。尝试状态存于宿主，重新加载页面会重新加入正在进行的尝试；已结束的尝试保持可见，直到下一次开始。登录成功后，该提供方走普通添加流程且密钥留空即可——provider 原生认证会解析已存储的授权。宿主没有注册登录流时（如未挂载 authorization 服务的组合），此区域不渲染。
