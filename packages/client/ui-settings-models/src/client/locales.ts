...
export const en = {
  nav: 'Models',
  title: 'Models',
  intro: 'Sign in with a supported account or configure an API key to use models.',
  edit: 'Edit',
  ...
  retry: 'Retry',
  accountAuthHint: 'Using account sign-in for model requests and model sync. Sign in above if needed.',
  apiKeyAuthHint: 'API key authentication overrides account sign-in. Switching to account sign-in removes the saved key reference immediately; other saved settings and credentials are kept.',
  useAccountAuth: 'Use account sign-in',
  useApiKeyAuth: 'Use an API key instead',
  accountAuthPinned: 'This deployment supplies an API key reference. Its configuration must change before account sign-in can be used.',
  accountAuthSaved: 'Saved: use account sign-in. Other edits in this card still need Apply.',
  accountAuthLoading: 'Account sign-in options are unavailable. Reload settings to retry, or explicitly choose an API key.',
  keyInput: 'API key',
...
export const zh: { [Key in keyof typeof en]: string } = {
  nav: '模型',
  title: '模型',
  intro: '登录支持的账号，或配置 API 密钥，即可使用模型。',
  edit: '编辑',
  ...
  retry: '重试',
  accountAuthHint: '模型请求和模型同步使用账号登录；如尚未登录，请使用上方登录入口。',
  apiKeyAuthHint: 'API 密钥会覆盖账号登录。切换为账号登录将立即移除已保存的密钥引用，保留其他已保存设置和凭据。',
  useAccountAuth: '使用账号登录',
  useApiKeyAuth: '改用 API 密钥',
  accountAuthPinned: '当前部署配置指定了 API 密钥引用，需先调整部署配置才能使用账号登录。',
  accountAuthSaved: '已保存：使用账号登录。此卡片中的其他编辑仍需保存。',
  accountAuthLoading: '账号登录选项尚未就绪。可重新打开设置重试，或明确选择使用 API 密钥。',
  keyInput: 'API 密钥',
