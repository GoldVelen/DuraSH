...
/**
 * Retired 0.1.1-era workflow run-state mirrors retained so DuraSH sessions keep
 * a contiguous seq. Same allowlist as stored-event validation; see
 * `.agents/notes/implemented/architecture/2026-08-30-retain-ignorable-external-session-events.md`.
 */
export const INERT_LEGACY_EVENT_TYPES: ReadonlySet<string> = new Set([
  'runs/dispatched',
  'workflow/start',
  'workflow/change',
])
...
function assertArtifactCoordinates(
  ...
  for (let index = 0; index < artifact.events.length; index += 1) {
    ...
    if (!currentKnown && !legacy && !ignorableCurrent && !vocabularyNeutral && !INERT_LEGACY_EVENT_TYPES.has(type)) {
