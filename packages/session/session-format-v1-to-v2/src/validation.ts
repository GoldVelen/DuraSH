...
import { INERT_LEGACY_EVENT_TYPES, assertReleasedArtifactRelationships } from '@deepseek-ai/dsh-session-format-v0-to-v1'
...
function validateReleasedV2Artifact(
  ...
  for (const [index, event] of artifact.events.entries()) {
    ...
    if (mode === 'current' && disposition === undefined && !installed && !ignorableUnknown && !INERT_LEGACY_EVENT_TYPES.has(type)) {
