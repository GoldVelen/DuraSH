...
The [global instruction editor](../../packages/context/agent-instructions/README.md#editing-global-rules) reads one fixed Host `AGENTS.md`, separately from the namespaced settings document. Its response reports file state and loading limits; a successful save is not evidence that a particular model request loaded the rules.
