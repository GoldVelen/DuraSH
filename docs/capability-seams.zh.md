...
flowchart LR
  ...
  svc_authorizationController["ctx.authorizationController<br/>Host sign-in-surface Remote controller"]
  ...
  svc_globalRules["ctx.globalRules<br/>Host global instruction editor"]
  ...
  pkg_agent_instructions --> svc_globalRules
  ...
  svc_fs --> pkg_tool_fs
  svc_globalRules --> pkg_api_settings_controller
...
| `ctx.authorizationController` | `core` | [`api-settings-controller`](../packages/api/settings-controller) | - | - | - | 把 authorization seam 投影到生成的 Remote namespace：长时间运行的尝试不带挂起请求启动，通知、提问与结算都在这里，而不在 seam Definition 上。 |
...
| `ctx.globalRules` | `core` | [`agent-instructions`](../packages/context/agent-instructions) | - | [`api-settings-controller`](../packages/api/settings-controller) | - | 通过版本检查读取并原子替换所配置的 Host AGENTS.md；保存成功后，使用同一文件的智能体会刷新指令上下文。 |
...
| `ctx.reliabilityPolicy` | `core` | `durash-reliability-policy` | - | `durash-tool-reliability` | - | 保存每个 Session 的启用状态与实施/审查模型选择，根据实时 LLM 目录校验已启用的选择器，并在已保存模型离开目录时关闭已启用的记录行。 |
