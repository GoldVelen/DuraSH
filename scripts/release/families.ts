...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@deepseek-ai/') && !name.startsWith('@durash/')) {
        throw new Error(`${normalized} must name an @deepseek-ai or @durash package`)
      }
...
class DshFamily extends ReleaseFamily {
  ...
  override members(root: string): ReleaseMember[] {
    const published = super.members(root)
    const privateNames = new Set<string>()
    for (const manifestPath of globSync([...this.patterns], { cwd: root })) {
      const manifest = readManifest(resolve(root, manifestPath.replaceAll('\\', '/')))
      if (manifest.private !== true) continue
      const name = manifest.name
      if (typeof name === 'string') privateNames.add(name)
    }
