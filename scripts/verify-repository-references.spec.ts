...
function sourceManifest(commit: string) {
  return {
    formatVersion: 1,
    sources: [
      { id: 'primary', name: 'Primary', kind: 'github', syncMode: 'primary-merge', baseline: commit, repository: 'example/primary', ref: 'main' },
      { id: 'vendor', name: 'Vendor', kind: 'npm', syncMode: 'vendored-review', baseline: '1.0.0', package: 'example', sourceRepository: 'example/vendor', snapshotBaseline: commit },
    ],
    registryDependencies: { sourceOfTruth: ['pnpm-lock.yaml'], updateOwner: 'Dependabot' },
  }
}
...
describe('maintained repository reference policy', () => {
  it('accepts full machine-owned upstream baselines and snapshot revisions', (test) => {
    const fixture = repository(test)
    fixture.write('UPSTREAM_SOURCES.json', JSON.stringify(sourceManifest(fixture.commit), null, 2))
    expect(scanRepositoryReferences(fixture.root)).toEqual([])
  })

  it('still checks other fields and nested baseline names in the source manifest', () => {
    const commit = 'a'.repeat(40)
    const manifest = sourceManifest(commit)
    manifest.sources[0]!.name = commit
    manifest.registryDependencies.updateOwner = commit
    const source = JSON.stringify({ ...manifest, nested: { baseline: commit }, snapshotBaseline: commit }, null, 2)
    const findings = findRepositoryReferences('UPSTREAM_SOURCES.json', source, new Set([commit]))
    expect(findings).toHaveLength(4)
    expect(findings.map(finding => source.split('\n')[finding.line - 1]?.trim())).toEqual([
      `"name": "${commit}",`,
      `"updateOwner": "${commit}"`,
      `"baseline": "${commit}"`,
      `"snapshotBaseline": "${commit}"`,
    ])
  })

  it('keeps same-line nonmetadata references and disallowed URLs visible', () => {
    const commit = 'b'.repeat(40)
    const manifest = sourceManifest(commit)
    manifest.sources[0]!.name = `${commit} ${organizationUrl}`
    expect(findRepositoryReferences('UPSTREAM_SOURCES.json', JSON.stringify(manifest), new Set([commit]))).toEqual([
      { file: 'UPSTREAM_SOURCES.json', line: 1, kind: 'organization-url' },
      { file: 'UPSTREAM_SOURCES.json', line: 1, kind: 'commit-hash' },
    ])
  })

  it('does not extend revision exceptions to another file or an invalid manifest', () => {
    const commit = 'c'.repeat(40)
    const commits = new Set([commit, commit.slice(0, 12)])
    const source = JSON.stringify(sourceManifest(commit))
    for (const file of ['other.json', 'nested/UPSTREAM_SOURCES.json']) {
      expect(findRepositoryReferences(file, source, commits)).toEqual([{ file, line: 1, kind: 'commit-hash' }])
    }
    const short = sourceManifest(commit)
    short.sources[0]!.baseline = commit.slice(0, 12)
    const npmCommit = sourceManifest(commit)
    npmCommit.sources[1]!.baseline = commit
    for (const invalid of [source.slice(0, -1), JSON.stringify({ baseline: commit }), JSON.stringify(short), JSON.stringify(npmCommit)]) {
      expect(findRepositoryReferences('UPSTREAM_SOURCES.json', invalid, commits))
        .toEqual([{ file: 'UPSTREAM_SOURCES.json', line: 1, kind: 'commit-hash' }])
    }
  })

  it('does not exempt ambiguous duplicate revision fields', () => {
    const commit = 'd'.repeat(40)
    const source = JSON.stringify(sourceManifest(commit))
      .replace(`"baseline":"${commit}"`, `"baseline":"${commit}","baseline":"${commit}"`)
    expect(findRepositoryReferences('UPSTREAM_SOURCES.json', source, new Set([commit])))
      .toEqual([{ file: 'UPSTREAM_SOURCES.json', line: 1, kind: 'commit-hash' }])
  })
