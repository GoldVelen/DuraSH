 ...
 *
 * The `@durash/dsh-*` overlay scope never had a wildcard: its packages are
 * generated from their declared manifest names, because the scope names its
 * directories non-uniformly (`durash-web-profile`, `ui-brand-durash`) and no
 * directory rule could reconstruct those specifiers.
 *
...
/** Package-name prefix whose aliases replay the removed group wildcards. */
const PREFIX = '@deepseek-ai/dsh-'

/** Package-name prefix of the DuraSH-owned overlay scope, mapped by declared name. */
const OVERLAY_PREFIX = '@durash/dsh-'
 ...
 * @returns Every directory whose manifest names a covered package and that carries `src`.
 */
function workspacePackages(): WorkspacePackage[] {
  ...
  for (const group of readdirSync(packages).sort()) {
    ...
    for (const directory of readdirSync(groupDir).sort()) {
      ...
      if (name === undefined || (!name.startsWith(PREFIX) && !name.startsWith(OVERLAY_PREFIX))) continue
 ...
 *
 * Unlike {@link collectPackageNames} this skips packages whose name does not
 * match their directory. The generator cannot map those under the wildcard
 * rule — only a hand-written alias can — but they still have to be mapped by
 * something, because deleting the group wildcards removed the fallback that
 * used to catch them. Overlay-scope packages never had a wildcard, so their
 * declared name alone is the rule.
 *
...
export function collectPackageAliases(): PackageAlias[] {
  ...
  for (const { group, directory, packageDir, name } of workspacePackages()) {
    const overlay = name.startsWith(OVERLAY_PREFIX)
    if (!overlay && name !== `${PREFIX}${directory}`) continue
    ...
    if (previous !== undefined) {
      throw new Error(
        ...
        + 'an explicit map cannot express two source directories for one specifier.',
      )
    }
 ...
 *
 * @returns Declared names of every covered package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@(?:deepseek-ai|durash)\/dsh-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@(?:deepseek-ai|durash)\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
