...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-brand-durash': { kind: 'none', reason: 'Browser-side presentation occupants; registers nothing model-facing.' },
  ...
  'packages/client/ui-settings-global-rules': { kind: 'none', reason: 'Browser-side file editor; agent-instructions owns the model-visible content and refresh.' },
  ...
  'packages/bundle/durash-web-profile': { kind: 'indirect', reason: 'The overlay mounts DuraSH identity, the reliability loop, the composer switch, and the gated handoff tool; those packages own any model-visible effect.' },
  'packages/reliability/durash-reliability-loop': { kind: 'indirect', reason: 'The runtime contributes no model context; every stage request is assembled by the workflow engine and its subagent providers.' },
  'packages/reliability/durash-reliability-policy': { kind: 'indirect', reason: 'The policy stores enablement and lane selectors; the handoff tool owns the model-visible guidance and schema.' },
  'packages/client/ui-reliability': { kind: 'indirect', reason: 'The composer chip writes Host policy; the handoff tool owns the model-visible guidance and schema that follow from enablement.' },
