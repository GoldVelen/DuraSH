...
运行命令时所在的目录将作为默认 workspace 根目录。公开安装的 `@deepseek-ai/dsh` 会在首次使用时自动初始化 `web`、`headless`、`sdk`、`sdk-minimal` 和 `acp`。DuraSH 源码 checkout 还会提供私有的 `durash` 模板；其他任何 profile 都必须通过 `dsh plugin` 创建。
