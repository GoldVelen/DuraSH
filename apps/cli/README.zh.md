...
| `dsh --profile durash` | 在上游 Web 栈之上启动 DuraSH Web 组合（仅限源码 checkout）。 |
| `dsh web` | 启动 Web profile。 |
| `dsh plugin --profile <name> <pnpm args>` | 通过在 profile 目录中转发给 pnpm 来管理该 profile 的插件。 |

运行命令时所在的目录将作为默认 workspace 根目录。公开安装的 `@deepseek-ai/dsh` 会在首次使用时自动初始化 `web`、`headless`、`sdk`、`sdk-minimal` 和 `acp`。DuraSH 源码 checkout 还会提供私有的 `durash` 模板。使用 `--from-default-profile` 可以基于这些模板之一，在尚未使用的非内置名称处创建其他 profile；通过 `dsh plugin` 则可以初始化一个以 base 为基础的 profile。`desktop` 名称保留给 Electron 持有的 profile，因此 CLI（命令行界面）会拒绝针对它的启动、配置 dump 和插件管理请求。
...
`dsh.profile.bundles` 中列出的组合包先从 dsh 安装目录解析（`@deepseek-ai/dsh-base`、`@deepseek-ai/dsh-web-app`、`@deepseek-ai/dsh-headless`、`@deepseek-ai/dsh-sdk-app`、`@deepseek-ai/dsh-sdk-minimal`、`@deepseek-ai/dsh-acp-app`），再从 profile 自身的 `node_modules` 解析；pnpm 会将树外插件安装到该目录。DuraSH 源码 checkout 还会通过源码自有的 `durash` 模板解析私有 `@durash/dsh-web-profile` 组合包；两者都不属于公开 CLI 安装。
...
生产运行需要已构建的包与前端产物。DuraSH checkout 使用 `pnpm run build` 后再运行 `pnpm start`；这两个命令会分别选择 DuraSH 客户端与运行 profile。如需上游源码 profile，请先运行 `pnpm run build:local`，再运行 `pnpm dsh web`；`pnpm dsh <args...>` 仍是其他具名 profile 使用的底层 TypeScript 启动器。模块解析约定以[源码执行参考](reference/README.zh.md#source-execution)为准。
