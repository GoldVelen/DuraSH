...
export function initializeProfileFromDefault(
  ...
  const templates = installationProfileTemplates()
  const template = Object.hasOwn(templates, fromDefaultProfile)
    ? templates[fromDefaultProfile]
    : undefined
  if (template === undefined) {
    const expected = Object.keys(templates).sort().map(value => JSON.stringify(value)).join(', ')
    throw new Error(
...
export function prepareProfile(name: string, userLayer = true, fromDefaultProfile?: string): Profile {
  ...
  const profile = loadProfile(NAME, name, INSTALL_ANCHOR, undefined, {
    userLayer,
    profileTemplates: installationProfileTemplates(),
  })
